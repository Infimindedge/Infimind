...
export function Hero() {
  ...
  return (
    <section className="overflow-hidden pt-12 pb-16 md:pt-16 md:pb-20 lg:pb-24">
      {/*
        Not wrapped in the standard <Container> — the photo intentionally
        breaks out of the normal content padding on its right edge and
        bleeds to the outer 1440px boundary with no border/shadow/rounding
        on that side, so it reads as part of the page rather than a
        separate framed card (per locked reference).
      */}
      <div className="mx-auto grid max-w-[1440px] grid-cols-1 items-center gap-12 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.05fr)] lg:gap-0">
        <motion.div
          variants={container}
          initial="hidden"
          animate="show"
          className="px-6 md:px-12 lg:pl-[72px] lg:pr-12"
        >
        ...
        <motion.div
          ...
          className="overflow-hidden lg:rounded-l-[18px]"
          style={{
            maskImage: 'linear-gradient(to right, transparent 0%, black 12%)',
            WebkitMaskImage: 'linear-gradient(to right, transparent 0%, black 12%)',
          }}
        >
          <motion.div style={{ x: translateX, y: translateY }} className="aspect-[16/10] w-full lg:aspect-auto lg:h-full">
            <Photo
